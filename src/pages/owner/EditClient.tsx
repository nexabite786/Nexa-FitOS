...
export function EditClient() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (clientSnap.exists()) {
        ...
        const trainersRef = collection(db, 'tenants', tenantId as string, 'trainers');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!tenantId || !clientId || !user) return;
    ...
    try {
      ...
      const clientSnap = await getDoc(clientRef);
      const previousTrainerId = clientSnap.exists() ? clientSnap.data().trainerId : null;
      ...
      if (previousTrainerId !== formData.trainerId) {
        let action = 'CLIENT_UNASSIGNED';
        if (formData.trainerId) {
          action = previousTrainerId ? 'CLIENT_REASSIGNED' : 'CLIENT_ASSIGNED';
        }
        await setDoc(doc(collection(db, 'tenants', tenantId, 'auditLogs'), Date.now().toString()), {
          action,
          performedBy: user.uid,
          targetId: clientId,
          details: { previousTrainerId, newTrainerId: formData.trainerId },
      ...
      setTimeout(() => {
        navigate(`/owner/clients/${clientId}`);
      }, 1500);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  if (loading) {
    return (
      <div className="flex justify-center py-20">
        <div className="animate-pulse text-muted-foreground tracking-widest uppercase text-sm">Loading Client Data...</div>
      </div>
    );
  }
  ...
  if (errorState === 'permission-denied') {
    return (
      <div className="text-center py-20 bg-card rounded-xl border border-border/50 mt-8">
        ...
        <p className="text-muted-foreground mt-2 mb-6">You don't have permission to edit this client.</p>
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 pb-12 animate-in fade-in duration-500">
      <div className="flex items-center gap-2 mb-2">
        <Button variant="ghost" size="sm" onClick={() => navigate(`/owner/clients/${clientId}`)} className="text-muted-foreground hover:text-foreground">
      ...
      <Card className="border-border/50 bg-card/50 backdrop-blur shadow-none">
        <CardHeader className="border-b border-border/50 pb-6">
          <CardTitle className="text-2xl font-display">Edit Client Profile</CardTitle>
          <CardDescription>Update member details and coach assignments.</CardDescription>
        </CardHeader>
        ...
        <form onSubmit={handleSubmit}>
          <CardContent className="space-y-8 pt-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 pt-4 border-t border-border/50">
              <div className="space-y-3">
                <Label htmlFor="timezone" className="text-muted-foreground uppercase text-xs tracking-wider">Timezone</Label>
              ...
              <div className="space-y-3">
                <Label htmlFor="trainerId" className="text-muted-foreground uppercase text-xs tracking-wider">Assigned Coach</Label>
                <div className="relative">
                  <select 
                    id="trainerId"
                    className="flex w-full appearance-none h-11 rounded-md border border-border bg-input/50 px-3 py-2 text-sm text-foreground shadow-sm focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring"
                  ...
                  >
                    ...
                    {trainers.map(t => (
                      <option key={t.id} value={t.id}>{t.firstName} {t.lastName} {t.status !== 'ACTIVE' ? `(${t.status})` : ''}</option>
                    ))}
                  </select>
                  <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-3 text-muted-foreground">
                    <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7"></path></svg>
          ...
          <CardFooter className="flex justify-end gap-3 border-t border-border/50 p-6 bg-card">
            <Button variant="ghost" type="button" onClick={() => navigate(`/owner/clients/${clientId}`)} className="h-11 px-6">
              Cancel
            </Button>
